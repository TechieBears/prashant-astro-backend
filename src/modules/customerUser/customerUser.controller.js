...
exports.createCustomerUser = asyncHandler(async (req, res, next) => {
  ...
  let imageName = generateImageName(req.files?.image?.[0]?.originalname || `https://ui-avatars.com/api/?name=${firstName}+${lastName}`);
  ...
  if (registerType === "google" || registerType === "apple") {
    ...
    try {
      ...
      if (referralCode) {
        referrer = await CustomerUser.findOne({ referralCode });
        if (!referrer) {
          await session.abortTransaction();
          session.endSession();
          return next(new ErrorHander("Invalid referral code", 400));
      ...
      // 2️⃣ Create Wallet with 0 balance initially
      const wallet = await Wallet.create(
        [
          {
            balance: 0,
        ...
        customerUser = await CustomerUser.findOneAndUpdate(
          { _id: existingUser.profile },
          {
            $set: {
              ...
              lastName,
              title,
              referredBy: referrer ? referrer._id : null // 👈 Update referrer info
            }
          },
          { new: true }
        );
...
// @desc    Get all customers for dropdown
// @route   GET /api/customer/dropdown
// @access  Private/Admin
exports.getAllCustomersDropdown = asyncHandler(async (req, res, next) => {
  const pipleine = [
    {
      $match: { role: "customer", isDeleted: false }
    },
  ...
  const users = await User.aggregate(pipleine);
  ...
  res.ok({ customers }, "Customers for dropdown fetched successfully");
});
